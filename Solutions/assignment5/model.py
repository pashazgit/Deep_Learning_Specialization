...
class MyNetwork(nn.Module):
    ...
    def __init__(self, config, input_shp, mean=None, std=None):
        ...
        for _i in range(config.num_hidden):
            ...
            setattr(self, "linear_{}".format(_i), nn.Linear(indim, outdim))
            setattr(self, "relu_{}".format(_i), nn.ReLU(True))
            indim = outdim
    ...
    def forward(self, x):
        ...
        for _i in range(self.config.num_hidden):
            x = getattr(self, "linear_{}".format(_i))(x)
            x = getattr(self, "relu_{}".format(_i))(x)
        x = self.output(x)
