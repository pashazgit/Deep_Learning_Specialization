...
def train(x_tr, y_tr, x_va, y_va, config):
    ...
    print("Initializing...")
    num_class = 10
    # TODO (4 points): Initialize W to very small random values. e.g. random
    # values between -0.001 and 0.001. To get results identical to the one in
    # the assignment package, you should initialize with uniform random numbers
    # between this range.
    W = 2e-3 * (np.random.rand(np.prod(x_tr_n.shape[1:]), num_class) - 0.5)
    ...
    for idx_epoch in range(num_epoch):
        # TODO (5 points): Create a random order to go through the data
        ind_data = np.random.permutation(len(x_tr_n))
        ...
        for idx_batch in range(num_batch):
            ...
            x_b = np.array([x_tr_n[_i] for _i in ind_cur])
        ...
        # TODO (5 points): If best validation accuracy, update W_best, b_best,
        # and best accuracy. We will only return the best W and b
        if acc > best_acc:
            best_acc = acc
            W_best = W
            b_best = b
    ...
    # TODO (2 points): Pack results. Remeber to pack pre-processing related
    # things here as well
    train_res = {}
...
def main(config):
    ...
    # TODO: Prepare folds. --- Included in the point below
    x_trva = np.reshape(x_trva, (num_fold, len(x_trva) // num_fold, -1))
    y_trva = np.reshape(y_trva, (num_fold, len(y_trva) // num_fold))
    # Here I've reshaped the data into 5x(N/5)xD, so that the first dimension
    # is the fold
    ...
    for idx_va_fold in va_fold_to_test:
        # TODO (10 points): Create train and validation data using folds
        tr_folds = list(range(idx_va_fold)) + \
            list(range(idx_va_fold + 1, num_fold))
        x_tr = np.concatenate([
            x_trva[_f] for _f in tr_folds
        ])
        x_va = x_trva[idx_va_fold]
        y_tr = np.concatenate([
            y_trva[_f] for _f in tr_folds
        ])
        y_va = y_trva[idx_va_fold]
    ...
    if config.cross_validate:
        ...
        va_acc_max = [max(_res["va_acc_epoch"]) for _res in train_res]
        print("Average best validation accuracy: {}%".format(
            np.mean(va_acc_max) * 100))
